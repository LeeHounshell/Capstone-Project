...
/*
 * Class for show playback with media control
 */
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    private ArrayList<Episode> mItems = new ArrayList<Episode>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mItems = new ArrayList<Episode>();
        mSelectedEpisode = (Episode) getActivity()
                .getIntent().getSerializableExtra(EpisodeTVActivity.EPISODE);

        List<Episode> episodes = EpisodeList.list;

        for (int j = 0; j < episodes.size(); j++) {
            mItems.add(episodes.get(j));
            if (mSelectedEpisode.getTitle().contentEquals(episodes.get(j).getTitle())) {
                mCurrentItem = j;
            }
        }
    ...
    private int getDuration() {
        Episode episode = mItems.get(mCurrentItem);
        MediaMetadataRetriever mmr = new MediaMetadataRetriever();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mmr.setDataSource(episode.getShowUrl(), new HashMap<String, String>());
        } else {
            mmr.setDataSource(episode.getShowUrl());
        }
    ...
    private void updatePlaybackRow(int index) {
        if (mPlaybackControlsRow.getItem() != null) {
            Episode item = (Episode) mPlaybackControlsRow.getItem();
            item.setTitle(mItems.get(mCurrentItem).getTitle());
            item.setStudio(mItems.get(mCurrentItem).getStudio());
        }
        if (SHOW_IMAGE) {
            updateShowImage(mItems.get(mCurrentItem).getCardImageURI().toString());
        }
    ...
    private void addOtherRows() {
        ...
        for (Episode episode : mItems) {
            listRowAdapter.add(episode);
        }
        HeaderItem header = new HeaderItem(0, getString(R.string.related_episodes));
    ...
    protected void updateShowImage(String uri) {
    ...
    public interface OnPlayPauseClickedListener {
        void onFragmentPlayPause(Episode episode, int position, Boolean playPause);
    }

    static class DescriptionPresenter extends AbstractDetailsDescriptionPresenter {
        @Override
        protected void onBindDescription(ViewHolder viewHolder, Object item) {
            viewHolder.getTitle().setText(((Episode) item).getTitle());
            viewHolder.getSubtitle().setText(((Episode) item).getStudio());
        }
    }
}
